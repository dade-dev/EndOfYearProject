...
public class NetworkService {
    ...
    public interface MessageListener {
        ...
        // Add new callback for connection events
    ...
    private boolean isConnectionValid(Socket socket) {
        ...
        // Additional check: try to read socket state (won't block but will detect closed sockets)
        try {
    ...
    private void removePeer(String ip) {
        ...
        if (outSocket != null) {
            try { outSocket.close(); } catch (IOException e) { /* ignore */ }
        }
        ...
        if (inSocket != null) {
            try { inSocket.close(); } catch (IOException e) { /* ignore */ }
        }
        inWriters.remove(ip);
    }
